...
@Component
public class CircuitWeatherClient implements WeatherClient {

    private static final String CLIENT_NAME = "[CircuitWeatherClient] ";

    private final WeatherClient kmaClient;
    private final WeatherClient openWeatherMapClient;
    private final ReactiveCircuitBreaker circuitBreaker;

    public CircuitWeatherClient(
    ...
    public Mono<WeatherForecastResponse> fetchWeather(int nx, int ny) {
        return circuitBreaker.run(
                ...
                throwable -> { // 2. 실패 시 Fallback 실행
                    log.warn(CLIENT_NAME + "⚠️ KMA API Failed! Switching to OpenWeatherMap... Error: {}", throwable.getMessage());
                    // 에러 발생 시 서브(OWM) 호출
