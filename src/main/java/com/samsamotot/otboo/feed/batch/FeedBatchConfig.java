...
public class FeedBatchConfig {
    ...
    // Step 정의
    @Bean
    public Step deleteSoftDeletedFeedsStep(
        JobRepository jobRepository,
        PlatformTransactionManager transactionManager,
        @Qualifier(FeedBatchConfig.READER_NAME) JdbcPagingItemReader<UUID> softDeletedFeedReader
    ) {
        return new StepBuilder(STEP_NAME, jobRepository)
            .<UUID, UUID>chunk(chunkSize, transactionManager)
            .reader(softDeletedFeedReader)
